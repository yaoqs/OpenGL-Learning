...
/** 输出立体文字 */
void GLFont::Print3DText(char *string,float z)
{
	HDC hDC = wglGetCurrentDC();         /**< 当前设备场景句柄 */
	HFONT hOldFont=(HFONT)::SelectObject(hDC,m_hFont); /**< 选择字体 */
	int   nListNum;                   /** 定义列表变量 */
	DWORD dwChar;                     /**< 字符指针 */
	GLYPHMETRICSFLOAT pgmf[1];        /**< 轮廓字体字符集的信息 */
	UCHAR * pChar=(UCHAR* )string;
	/** 压入堆栈 */
	glPushMatrix();
	glDisable(GL_LIGHTING); /**< 关闭光照 */
	...
	{ 
		if( IsDBCSLeadByte((BYTE)pChar[i]) )   /**< 是否为双字节(汉字) */
		{ 
			dwChar=(DWORD)((pChar[i]<<8)|pChar[i+1]);  /**< 取当前字节,双字节变换 */
		    i++;
		}
	  else	 
